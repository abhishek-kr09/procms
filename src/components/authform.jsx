...
export default function AuthForm({ origin }){
    ...
    return <div className="w-full sm:w-1/2 md:w-1/5 mx-4 p-4 rounded-lg bg-zinc-800 flex flex-col items-center gap-4">
            <Album className="size-12 text-gray-300" />
            <p className="text-center text-sm text-gray-200">{origin == 'signup' ? 'Welcome, by continuing with ProCMS sign in, you\'ll be a Geek' : "Welcome back, hope to have you back!" } </p>
            ...
            <p className="text-sm text-gray-400 text-center">Already having an account? <Link className="underline" href="/sign-in">Sign In</Link></p>
             : 
            <p className="text-sm text-gray-400">New to ProCMS? <Link className="underline" href="/sign-up">Sign up</Link></p>
            }
        </div>
}